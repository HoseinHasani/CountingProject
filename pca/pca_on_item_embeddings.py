...
token_mode = "element"          # "element", "separator"
...
def apply_pca_and_plot(ax, data, token_mod, layer_name, cmap):
    ...
    ax.set_title(layer_name, fontsize=16, pad=2)
    ax.set_xticks([])
    ax.set_yticks([])
...
handles = [
    plt.Line2D([], [], color=cmap(i / 8), marker='o', linestyle='', label=f"Token {i+1}")
    for i in range(9)
]
# if token_mode == "both":
#     handles += [
...
fig.legend(handles=handles, loc="lower center", ncol=9, prop={"weight": "bold", "size": 16},
           frameon=False, fontsize=16, bbox_to_anchor=(0.5, 0.02))

fig.suptitle(f"PCA of Item Token Embeddings ({token_mode.capitalize()})",
             fontsize=26, weight="bold", y=0.97)

plt.tight_layout()
plt.subplots_adjust(top=0.93, bottom=0.05, left=0.04, right=0.98)

out_name = f"layerwise_pca_llm_{token_mode}_pe.pdf"
